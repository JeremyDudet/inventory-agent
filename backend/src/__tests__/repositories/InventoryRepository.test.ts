...
describe('InventoryRepository', () => {
  ...
  describe('findById', () => {
    it('should return the item when found by ID', async () => {
      // Arrange
      const mockItem = { id: '123', name: 'Test Item', quantity: 10, unit: 'kg', category: 'Test' };
      const mockQuery = {
        select: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
    ...
    it('should handle database errors gracefully', async () => {
      ...
      // Assert
      expect(result).toBeNull();
      expect(console.error).toHaveBeenCalledWith('Error finding item by ID:', mockError);
  ...
  describe('findByName', () => {
    it('should find items by name with case-insensitive partial match', async () => {
      // Arrange
      const mockItems = [{ id: '1', name: 'Test Item 1' }, { id: '2', name: 'Test Item 2' }];
      const mockQuery = {
        select: jest.fn().mockReturnThis(),
        ilike: jest.fn().mockReturnThis(),
  ...
  describe('updateQuantity', () => {
    ...
    it('should handle database errors during update', async () => {
      ...
      // Assert
      expect(console.error).toHaveBeenCalledWith('Error updating item quantity:', mockError);
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('create', () => {
    it('should create a new inventory item successfully', async () => {
      // Arrange
      const newItem = { name: 'New Item', quantity: 10, unit: 'kg', category: 'Test', lastupdated: expect.any(String) };
    ...
    it('should handle invalid data', async () => {
      // Arrange
      const invalidItem = { name: '', quantity: -5, unit: 'kg', category: 'Test' };
  
      // Act & Assert
      await expect(repository.create(invalidItem)).rejects.toThrow('Item name is required and must be a non-empty string');
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should handle deletion errors', async () => {
      ...
      // Assert
      expect(console.error).toHaveBeenCalledWith('Error deleting inventory item:', mockError);
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('getCategories', () => {
    it('should return unique categories', async () => {
      // Arrange
      const mockItems = [{ category: 'Category 1' }, { category: 'Category 2' }, { category: 'Category 1' }];
      const mockQuery = {
        select: jest.fn().mockReturnThis(),
        order: jest.fn().mockReturnThis(),
        limit: jest.fn().mockResolvedValue({ data: mockItems, error: null }),
