...
describe('sessionLogsService', () => {
  ...
  describe('logTranscript', () => {
    it('should successfully log a transcript with a valid UUID user ID', async () => {
      ...
      (createTranscriptLog as jest.Mock).mockResolvedValueOnce({ success: true });

      const result = await logTranscript(text, isFinal, confidence, validUserId);
    ...
    it('should handle non-UUID user IDs by omitting the user ID field', async () => {
      ...
      const result = await logTranscript(text, isFinal, confidence, nonUuidUserId);
    ...
    it('should handle database errors gracefully', async () => {
      ...
      (createTranscriptLog as jest.Mock).mockRejectedValueOnce(new Error('Database error'));
