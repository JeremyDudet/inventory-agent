...
export interface ConfirmationRequest {
  ...
  previousConfirmations?: {
    // History of user's confirmations
...
class ConfirmationService {
  ...
  determineConfirmationType(request: ConfirmationRequest): ConfirmationResult {
    ...
    if (similarItems && similarItems.length > 0) {
      const isConfusable = similarItems.some(
        (item) => this.calculateSimilarity(request.item, item) > 0.7
      );
      ...
      if (isConfusable) {
        confirmationType =
          confirmationType === ConfirmationType.IMPLICIT
            ...
            : confirmationType;
        riskLevel =
          Math.max(this.riskLevelToNumber(riskLevel), 1) === 2
            ? "high"
            : "medium";
    ...
    if (
      request.isLargeQuantity ||
      (action !== "unknown" &&
        this.isLargeQuantityChange(
          action as "add" | "remove" | "set",
    ...
    if (
      ...
      threshold !== undefined &&
      currentQuantity - (typeof quantity === "number" ? quantity : 0) <
        threshold
    ) {
      confirmationType = ConfirmationType.EXPLICIT;
    ...
    if (request.previousConfirmations) {
      ...
      if (total > 5) {
        ...
        if (errorRate > 0.3) {
          confirmationType =
            Math.max(this.confirmationTypeToNumber(confirmationType), 2) >= 3
          ...
          if (recentMistakes && recentMistakes.length > 0) {
            if (recentMistakes.includes("quantity")) {
              suggestedCorrection =
                suggestedCorrection ||
                `Did you mean a different quantity than ${quantity}?`;
            } else if (recentMistakes.includes("item")) {
              suggestedCorrection =
                suggestedCorrection ||
                "Please confirm the item name is correct";
            }
          }
  ...
  private confirmationTypeToNumber(type: ConfirmationType): number {
    switch (type) {
      case ConfirmationType.IMPLICIT:
        return 0;
      case ConfirmationType.VOICE:
        return 1;
      case ConfirmationType.VISUAL:
        return 2;
  ...
  recordConfirmationResult(
    ...
    console.log(
      `User ${userId} confirmation result recorded: ${
    ...
    if (!wasCorrect && details) {
      ...
      if (details.mistakeType && details.correctedCommand) {
        ...
        if (details.mistakeType === "item" && details.correctedCommand.item) {
          ...
          console.log(
            `Item confusion: ${details.originalCommand.item} was confused with ${details.correctedCommand.item}`
          );
        ...
        if (
          details.mistakeType === "quantity" &&
          details.correctedCommand.quantity !== undefined
        ) {
          // Track quantity error patterns (e.g., consistently saying numbers incorrectly)
          console.log(
            `Quantity error: ${details.originalCommand.quantity} should have been ${details.correctedCommand.quantity}`
          );

          // Check for patterns like consistently being off by a factor
          const factor =
            details.correctedCommand.quantity /
            details.originalCommand.quantity;
          if (factor === 10 || factor === 0.1) {
            console.log(
              "User might have an order of magnitude confusion pattern"
            );
          }
        }
      }
  ...
  processVoiceCorrection(
    ...
    if (
      /^(yes|yeah|correct|right|that's right|sounds good|fine|okay|ok|yep|yup)$/i.test(
        correctionTextLower
      )
    ) {
    ...
    if (
      /^(no|nope|incorrect|wrong|that's wrong|not right)$/i.test(
        correctionTextLower
      )
    ) {
    ...
    const quantityMatch = correctionTextLower.match(
      /\b(?:no,?\s+)?(?:it'?s|it\s+should\s+be|make\s+it|should\s+be|i\s+meant|i\s+said|actually)?\s*(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\b/i
    );
    ...
    if (quantityMatch) {
      ...
      if (/^\d+$/.test(quantityMatch[1])) {
        ...
        correctedQuantity =
          wordToNumber[quantityMatch[1]] || originalCommand.quantity;
      }
    ...
    if (/(not|don't)\s+(add|remove|set)/i.test(correctionTextLower)) {
      ...
      if (
        /add/i.test(correctionTextLower) &&
        originalCommand.action !== "add"
      ) {
        newAction = "add";
      } else if (
        /remove/i.test(correctionTextLower) &&
        originalCommand.action !== "remove"
      ) {
        newAction = "remove";
      } else if (
        /set/i.test(correctionTextLower) &&
        originalCommand.action !== "set"
      ) {
        newAction = "set";
      }
    ...
    const unitMatch = correctionTextLower.match(
      /\bnot\s+(.+?),?\s+(?:but|it'?s|i\s+meant)\s+(.+?)(?:\s|$|\.)/i
    );
    if (
      unitMatch &&
      ["pounds", "boxes", "gallons", "bottles", "cans", "bags", "units"].some(
        (unit) => unitMatch[1].includes(unit) || unitMatch[2].includes(unit)
      )
    ) {
      // Determine which part has the unit
      let newUnit = "";
      for (const unitWord of [
        "pounds",
        "boxes",
        "gallons",
        "bottles",
