...
describe("Voice Command Processing", () => {
  ...
  describe("Inventory Updates", () => {
    test("should successfully update inventory with valid command", async () => {
      ...
      (inventoryService.updateInventoryCount as jest.Mock).mockResolvedValue(
        undefined
      );

      await inventoryService.updateInventoryCount(command);

      expect(inventoryService.updateInventoryCount).toHaveBeenCalledWith(
        command
      );
    });
    ...
    test("should handle missing quantity gracefully", async () => {
      ...
      (inventoryService.updateInventoryCount as jest.Mock).mockRejectedValue(
        new Error("Invalid quantity")
      );

      await expect(
        inventoryService.updateInventoryCount(command)
