...
describe("InventoryService", () => {
  ...
  describe("findBestMatch", () => {
    test("should find exact match", async () => {
      ...
      const expectedItem = items.find(
        (item) => item.name === "Coffee 16 ounce"
      );
      ...
      jest.spyOn(mockRepository, "findSimilarItems").mockResolvedValue([
        ...
        {
          item: items.find(
            (item) => item.name === "Coffee 8 ounce"
          ) as InventoryItem,
          similarity: 0.85,
        },
        {
          item: items.find((item) => item.name === "Coffee") as InventoryItem,
          similarity: 0.8,
        },
      ]);
    ...
    test("should find partial match", async () => {
      ...
      jest.spyOn(mockRepository, "findSimilarItems").mockResolvedValue([
        { item: expectedItem as InventoryItem, similarity: 0.9 },
        {
          item: items.find(
            (item) => item.name === "Coffee 8 ounce"
          ) as InventoryItem,
    ...
    test("should throw NotFoundError when no match is found", async () => {
      ...
      await expect(
        inventoryService.findBestMatch(extractedItem)
    ...
    test("should throw ValidationError for ambiguous matches below threshold", async () => {
      ...
      const similarItems = [
        ...
        {
          item: items.find(
            (item) => item.name === "Dark Roast Coffee 16 ounce"
          ),
          similarity: 0.55,
        },
      ];
      ...
      jest
        .spyOn(mockRepository, "findSimilarItems")
        .mockResolvedValue(
          similarItems as { item: InventoryItem; similarity: number }[]
        );
  ...
  describe("updateInventoryCount", () => {
    test("should update inventory quantity and broadcast WebSocket message", async () => {
      ...
      jest
        .spyOn(inventoryService, "findBestMatch")
        .mockResolvedValue(itemToUpdate);
      ...
      expect(await mockRepository.findById(itemToUpdate.id)).toHaveProperty(
        "quantity",
        expectedNewQuantity
      );
  ...
  describe("updateItem", () => {
    ...
    test("should throw ValidationError when update fails", async () => {
      jest
        ...
        .mockResolvedValue(null as unknown as InventoryItem);

      await expect(
        inventoryService.updateItem("non-existent-id", { quantity: 20 })
      ).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe("addItem", () => {
    ...
    test("should throw ValidationError when creation fails", async () => {
      jest
        ...
        .mockRejectedValue(new Error("Failed to create item"));
  ...
  describe("deleteItem", () => {
    ...
    test("should throw ValidationError when deletion fails", async () => {
      ...
      await expect(
        inventoryService.deleteItem("non-existent-id")
      ).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe("getItemQuantity", () => {
    ...
    test("should convert quantity to requested unit when possible", async () => {
      ...
      jest
        .spyOn(unitConversions, "getUnitType")
        .mockImplementation((unit) =>
          unit === "bags" || unit === "units" ? "count" : "unknown"
        );

      jest
        .spyOn(unitConversions, "convertQuantity")
        .mockImplementation(() => 20); // 10 bags = 20 units

      const result = await inventoryService.getItemQuantity(
        "Coffee 16 ounce",
        "units"
      );
